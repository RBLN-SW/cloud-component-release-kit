...
#
#   scripts/tag-release.sh <release-X.Y.Z> rc     # vX.Y.Z-rc<N+1>
#   scripts/tag-release.sh <release-X.Y.Z> ga     # vX.Y.Z
#
...
#     when RC_VALIDATION_ENFORCE=true, warn-only otherwise). The release
#     workflow re-checks both before anything is published, so a mistaken tag
#     never reaches a registry.
#
# The tag push is the trigger for the release workflow; this script publishes
# nothing.
...
echo "tagged $tag (the release workflow's ${kind} path is now running)"
