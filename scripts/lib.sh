...
#   - The remote is called origin, and the scripts run inside a checkout of the
#     repository being released (the caller repository in CI).
...
# grep exits 1 on no match, which pipefail would turn into a failure of the
# whole assignment in the caller; a repository without a GA yet is normal.
last_ga_tag() {
	git tag -l 'v[0-9]*' | { grep -Ev -- '-rc[0-9]+$' || true; } | sort -V | tail -1
}
...
prev_ga_tag() {
	local cur=$1
	{
		git tag -l 'v[0-9]*' | { grep -Ev -- '-rc[0-9]+$' || true; }
		echo "$cur"
...
# owner/name of the repository: GITHUB_REPOSITORY in CI, gh's view of origin
# when run by hand. Empty if neither is available.
repo_slug() {
