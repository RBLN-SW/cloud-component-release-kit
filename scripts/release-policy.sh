...
#   7  size                        >SIZE_LIMIT changed lines outside             comment only
#                                  GENERATED_PATHS
#
# Inputs (environment): BASE_REF PR_NUMBER PR_TITLE PR_BODY, PR_LABELS (newline
# separated) or PR_LABELS_JSON (JSON array), GENERATED_PATHS (pathspecs excluded
# from the size count, space separated, default "vendor go.sum *zz_generated*"),
# SIZE_LIMIT (default 400), GITHUB_REPOSITORY GH_TOKEN (for the size comment).
# HEAD is the PR head checkout.
...
# read does not glob, so "*zz_generated*" survives as a pathspec pattern.
read -r -a generated <<<"${GENERATED_PATHS:-vendor go.sum *zz_generated*}"
excludes=()
for p in ${generated[@]+"${generated[@]}"}; do excludes+=(":!$p"); done
...
# 7. size (advisory)
changed=$(git diff --numstat "$base...HEAD" -- . ${excludes[@]+"${excludes[@]}"} |
