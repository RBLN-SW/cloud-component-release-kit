#!/usr/bin/env bash
# Classify a pushed version tag for the release workflow and enforce the
# invariants that make "what was validated is what ships" true.
#
#   scripts/classify-tag.sh <tag>      # prints key=value lines for $GITHUB_OUTPUT
#
...
#                            whatever validates rcs for this repository).
#                            Anything else fails here, before a single artifact
#                            is published.
#
# The status check is enforced when RC_VALIDATION_ENFORCE=true and only warns
# otherwise, so this gate can ship before a validation pipeline reports.
