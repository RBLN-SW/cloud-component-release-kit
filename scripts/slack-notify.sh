...
#   scripts/slack-notify.sh --color <good|warning|danger|info|#hex> \
#       --title "<mrkdwn>" [--details "<mrkdwn>"] [--link "<url>|<label>"]... \
#       "<body mrkdwn, may span lines, may be empty>"
#
# The one-argument form posts plain text (with the run link appended). The
# flagged form posts a coloured attachment: a bold title, the body, an optional
# details block and a context line with the workflow run and any extra links.
# Bodies are mrkdwn, so `code`, *bold* and bullets (•) render.
#
# --details is a context block: Slack renders it in the small font with small
# emoji. Status rows (":white_check_mark: Images ...") belong there; a regular
# section would show every emoji at full size. Use emoji shortcodes, not
# unicode.
#
...
# above the attachment, which would show the title twice.
#
# Reads SLACK_OAUTH_TOKEN and SLACK_CHANNEL_ID. A missing token or a Slack
# error is reported as a workflow notice/warning and never fails the calling
# job: a lost notification must not block a release.
...
if [ -z "$title" ]; then
	...
	ctx=""
	for c in ${context[@]+"${context[@]}"}; do ctx="${ctx:+$ctx  ·  }$c"; done
