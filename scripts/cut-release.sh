...
#
#   scripts/cut-release.sh <minor|patch|major> [--version X.Y.Z]
#
...
# The rc tag push is what starts the rc path of the release workflow; this
# script itself builds nothing.
#
# A patch is refused unless its parent is the newest GA: published versions
# must stay in linear order (the latest alias and any upgrade graph built from
# the tags assume it). Requires push rights on origin (the release GitHub App
# in CI, or a bypass-listed release manager locally) and an authenticated gh.
...
if [ "$kind" = patch ]; then
	...
	parent="v$x.$y.$((z - 1))"
	[ "$parent" = "$last" ] ||
		fail "$version would patch $parent, but the newest GA is $last. Only the newest GA can be patched; published versions must stay in linear order."
fi
...
cut complete
  ...
  tag    : $tag  (the release workflow's rc path is now running)
